...
if __name__ == '__main__':
    ...
    k = torch.randn(batch_size, n_head, seq_len, head_embd, dtype=torch.float16).cuda().requires_grad_(True)
    v = torch.randn(batch_size, n_head, seq_len, head_embd, dtype=torch.float16).cuda().requires_grad_(True)

    l = torch.zeros(batch_size, n_head, seq_len, dtype=torch.float16).cuda()
    m = torch.full((batch_size, n_head, seq_len), float('-inf'), dtype=torch.float16).cuda()

    print('=== profiling manual attention forward ===')
    with torch.autograd.profiler.profile(use_cuda=True) as prof:
        manual_result_fp16 = manual_attn_fp16(q, k, v)
    ...
    manual_result = manual_result_fp16.detach().cpu().numpy()
    minimal_result = O.detach().cpu().numpy()
    err = np.max(abs(minimal_result - manual_result))
    print(f'{err=}')
    if err < 1e-1:
        print(f"fp16 forward: {GREEN}PASS{RESET}")
    else:
        print(f"fp16 forward: {RED}FAILED{RESET}")

    print('=== profiling manual attention backward ===')
    with torch.autograd.profiler.profile(use_cuda=True) as prof:
        torch.sum(manual_result_fp16).backward()
    prof_data = prof.key_averages()
    total_cuda_time = sum(item.cuda_time_total for item in prof_data)
    print(f"profiling manual attention backward: {total_cuda_time=} μs\n")
    #print(prof.key_averages().table(sort_by='cuda_time_total', row_limit=10))

    dO = torch.ones(batch_size, n_head, seq_len, head_embd, dtype=torch.float16).cuda()
    ...
    if (q_err < 1e-1) and (k_err < 1e-1) and (v_err < 1e-1):
        print(f"f16 backward: {GREEN}PASS{RESET}")
    else:
        print(f"fp16 backward: {RED}FAILED{RESET}")
